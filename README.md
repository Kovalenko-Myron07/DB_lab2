...
2. **Room (Приміщення):** Довідник кабінетів. `RoomID` — первинний ключ. (Category & Room: Базові довідники для класифікації майна та обліку кабінетів.)
3. **Employee (Співробітники):** Довідник персоналу з ПІБ та посадами. `EmployeeID` — первинний ключ. (Реєстр матеріально відповідальних осіб із зазначенням посад (наприклад, Senior Java Developer))
4. **Asset (Майно):** Основна таблиця. `AssetID` (PK). Посилається на категорію `CategoryID` (FK). (Центральна таблиця активів. Використовує унікальні інвентарні номери (UNIQUE) та контроль вартості (CHECK Price >= 0))
5. **Asset_Allocation (Журнал обліку):** Асоціативна таблиця. Поєднує майно, співробітника та приміщення. Має обмеження `CHECK (ReturnDate IS NULL OR ReturnDate >= IssueDate)`. (Журнал руху майна. Пов’язує активи, персонал та локації через зовнішні ключі (FK). Містить логічний контроль: дата повернення не може бути раніше видачі)
